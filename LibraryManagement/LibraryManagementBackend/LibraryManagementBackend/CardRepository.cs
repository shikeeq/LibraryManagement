...
('David Wilson', 'Chemical Engineering', 'S');";
        ...
        public List<Card> GetAllCards()
        {
            ...
            const string sql = "SELECT card_id, name, department, type FROM card";
        ...
        {
            const string sql = "SELECT card_id, name, department, type FROM card WHERE card_id = ?";
            using (var connection = GetConnection())
            using (var command = new OdbcCommand(sql, connection))
            {
        ...
        {
            const string sql = "INSERT INTO card (name, department, type) VALUES (?, ?, ?)";
            using (var connection = GetConnection())
            using (var command = new OdbcCommand(sql, connection))
            {
                command.Parameters.AddWithValue("?", card.Name);
                command.Parameters.AddWithValue("?", card.Department);
                command.Parameters.AddWithValue("?", card.Type.ToString());
                connection.Open();
                command.ExecuteNonQuery();
            }
        }
        ...
        {
            const string sql = "UPDATE card SET name = ?, department = ?, type = ? WHERE card_id = ?";
            using (var connection = GetConnection())
            using (var command = new OdbcCommand(sql, connection))
            {
                command.Parameters.AddWithValue("?", card.Name);
        ...
        {
            const string sql = "DELETE FROM card WHERE card_id = ?";
            using (var connection = GetConnection())
            using (var command = new OdbcCommand(sql, connection))
            {
                command.Parameters.AddWithValue("?", cardId);
